 ...
 *
 * Reported from play as "pressing keys doesn't always register". The cause is a cadence
...
function harness(displayHz: number, stepHz = 60) {
  let now = 0;
  ...
  const loop = new Loop(host, { stepHz });
  loop.start();
