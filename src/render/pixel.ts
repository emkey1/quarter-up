 ...
 *
 * This exists because the earlier art was drawn with ctx.arc() and ctx.fillRect() at
 * *display* resolution — smooth, anti-aliased vector shapes that never read as pixel
 * art no matter how the palette is tuned. The fix is not more colours or a bigger
 * canvas; it is authoring an actual grid of pixels and blitting it nearest-neighbour.
 *
 * Everything here works on palette INDICES, not colours, so a sprite can be recoloured
 * (monster levels, class tints) by swapping a ramp rather than redrawing it — which is
 * what keeps the art budget sane.
 */
 ...
 * Five steps is the sweet spot at 32x32: enough to model a form, few enough to stay
 * readable when the sprite is 32 pixels tall and moving.
 */
...
export const enum P {
  ...
  Light = 5,
  Lightest = 6,
  /** Second ramp starts here — used for trim, skin, metal, etc. */
  Outline2 = 7,
  Darkest2 = 8,
  Dark2 = 9,
  Base2 = 10,
...
export class Px {
  ...
  /** Filled ellipse. The workhorse: heads, bodies, blobs. */
   ...
   * gives the sprite a hard edge that survives being drawn over a busy floor.
   */
  ...
  /**
   * Light from the top-left: lift pixels whose up-left neighbour is empty, darken
   * those whose down-right neighbour is empty. Applied to whole ramps so it works on
   * any recolour.
   */
